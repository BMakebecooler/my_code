...
class Color
{
    ...
    const COLOR_BX_CONTENT_ID = 238;
    const COLOR_CONTENT_ID = 195;
    const COLOR_BX_PROPERTY_ID = 284;
    const COLOR_MAIN_PROPERTY_ID = 231;

    public static $badColorIds = [
        12863431,
        1026791
    ];

    public static $colorsHex = [
        'абрикосовый' => '#FBCEB1',
        ...
        'зеленый лишайник, мох' => '#ADDFAD',
        'цвет зеленого мха' => '#ADDFAD',
        ...
        'темно-алый' => '#CB2821',
        'темно-баклажановый' => '#660044',
        'темно-бежевый' => '#E8E8B3',
    ...
    /**
     * Для имени цвета возвращает сответствующий ему hex-код
     * @param $color
     * @return bool|mixed
     */
    public static function getHexFromName($color)
    {
        return self::$colorsHex[StringHelper::strtolower(trim($color))] ?? false;
    }

    public static function getFilterColors()
    {
        $return = \Yii::$app->cache->get('filter-colors');

        if ($return === false) {

            $return = [];
            $sql = "SELECT ccep.element_id,cce.name,cce.code FROM `cms_content_element_property` ccep
            ...
            foreach ($rows as $row) {
                $return[$row['element_id']] = $row['name'];
            }
            \Yii::$app->cache->set('filter-colors', $return);
        }
        $return[1026881] = 'Зеленый';
        $return[1026878] = 'Желтый';
        $return[1026898] = 'Красный';
        $return[1027025] = 'Черный';
        return $return;

    }

    public static function getColorsByFilterColorId(int $elementId)
    {
        $return = \Yii::$app->cache->get('colors_by_filter_color_id_' . $elementId);
        if ($return === false) {

            $sql = 'select ccer.content_element_id ,ccep.value, cce.name
            from cms_content_element_relation  ccer
            left join cms_content_element cce on cce.id = ccer.content_element_id
...
//                print_r($row);
//                echo '</pre>';
                $return[] = $row['content_element_id'];

                $rowsSecond = \Yii::$app->db->createCommand($sql, [
                    ':elementId' => $row['content_element_id']
                ])->queryAll();

                foreach ($rowsSecond as $rowSecond) {
                    $return[] = $rowSecond['content_element_id'];

//                $rowsTr = \Yii::$app->db->createCommand($sql,[
//                    ':elementId' => $rowSecond['content_element_id']
//                ])->queryAll();

//                foreach ($rowsTr as $rowTr){
//                    $return[] = $rowTr['content_element_id'];
//                }
...
//            echo '</pre>';
            ...
            \Yii::$app->cache->set('colors_by_filter_color_id_' . $elementId, $return);
        }
    ...
    public static function getFilterColorsByColorId(int $elementId)
    {
        $sql = 'select related_content_element_id from cms_content_element_relation  where content_element_id=' . $elementId;

        $rows = \Yii::$app->db->createCommand($sql, [])->queryAll();

        foreach ($rows as $row) {
            $sql =
                "select cce.id, cce.name, ccer.related_content_element_id,ccep.value
                from cms_content_element_relation ccer
                left join cms_content_element_property ccep on ccer.related_content_element_id=ccep.element_id
                left join cms_content_element cce on ccer.related_content_element_id = cce.id
                where ccer.content_element_id = :content_element_id 
                AND (ccep.value = 'filter' OR ccep.value != 'filter')";

            $rowsFilter = \Yii::$app->db->createCommand($sql, [
                ':content_element_id' => $row['related_content_element_id'],
            ])->queryAll();
    ...
    public static function getEtalonColors()
    {
        $return = [];
        $typeParam = ProductParamType::find()->where(['code' => 'KFSS_COLOR'])->one();
        if ($typeParam) {
