...
//            [['weight'], 'integer'],
    ...
    public function addData()
    {
        ...
        if ($tree->isNewRecord) {
            ...
            //$tree->level = $catalog->level + 1;
            //$tree->pid = $catalog->id;
            //$tree->pids = $catalog->pids . Tree::PIDS_DELIMETR . $catalog->id;
            //$tree->dir = $catalog->dir . '/' . $tree->code;
        ...
        if ($this->parentGuid) {
            ...
            if ($parentTree = Guids::getEntityByGuid($this->parentGuid)) {
                ...
                //$tree->pids = $parentTree->pids . Tree::PIDS_DELIMETR . $parentTree->id;
                //$tree->dir = $parentTree->dir . '/' . $tree->code;
                ...
                /*$parentTree = new Tree();
                $parentTree->guid->setGuid($this->parentGuid);
                $parentTree->name = uniqid();
                $parentTree->site_code = self::SITE_CODE;

                if ($parentTree->save()) {
                    $tree->pid = $parentTree->id;
                } else {

                    var_dump($parentTree->getErrors());
