...
namespace modules\api\resource\lapa\v2;
...
//            },
            'price' => function () {
                //Тут нужно вычислять минимульную цену модификаций данного товара типа Цена Лендинга 1

                //Выясним ID модификаций пригодных для продажи (у которых будем брать потом цену)
                $offersCanSaleQuery = ProductModel::getProductOffersCanSaleQuery($this->parent_content_element_id);
                $offersCanSaleQuery->select(['cms_content_element.id'])->andWhere(['card.id' => $this->id]);
                $offersCanSaleIds = $offersCanSaleQuery->column();

                $cardPrice = 0;
                if ($offersCanSaleIds){
                    $cardPriceLp = ShopProductPrice::find()
                        ->andWhere([
                            'type_price_id' => ShopTypePrice::PRICE_TYPE_LP1_ID,
//                            'type_price_id' => ShopTypePrice::PRICE_TYPE_DISCOUNTED, //Для тестов если нет цены ЛП1
                            'product_id' => $offersCanSaleIds
                        ])
                        ->min('price');

                    $cardPriceBase = ShopProductPrice::find()
                        ->andWhere([
                            'type_price_id' => ShopTypePrice::PRICE_TYPE_BASE_ID_OLD,
                            'product_id' => $offersCanSaleIds
                        ])
                        ->min('price');

                    //Если нашлась какая то из цен - имеет смысл выяснять какую используем
                    if ($cardPriceLp || $cardPriceBase){
                        //Если есть обе цены - берем наименьшую
                        if ($cardPriceLp && $cardPriceBase){
                            $cardPrice = min($cardPriceLp, $cardPriceBase);
                        }else{
                            $cardPrice = $cardPriceLp ?: $cardPriceBase;
                        }
                    }
                }

                return (string)intval($cardPrice);
            },
